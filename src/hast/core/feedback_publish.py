...
def publish_feedback_backlog(
    ...
    *,
    ...
    eligible = [
        ...
        and (
            lane is None
            or str(item.get("lane") or "project") == lane
        )
    ]
...
def _build_issue_title(item: dict[str, Any]) -> str:
    title = str(item.get("title") or "hast feedback item").strip()
    return f"[hast-feedback] {title}"[:200]
...
def _build_issue_body(item: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "_Generated by hast manager feedback publisher._",
    ]
    return "\n".join(lines)
