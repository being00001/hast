...
```
사람 → hast auto → claude 세션 1 → claude 세션 2 → ...
         ↑ 사람이 실행
```
...
Claude (오케스트레이터 세션)
  ├── hast.goals로 목표 분해
  ├── hast.context로 컨텍스트 조립
  ├── Task tool로 서브 에이전트에게 각 goal 위임
  ├── hast.git으로 롤백/커밋
  ├── hast.session으로 세션 간 상태 브릿지
  └── 실패하면 실패 컨텍스트 + 재위임
```

### 왜 이게 더 나은가

| | hast auto (현재) | 오케스트레이터 패턴 |
|---|---|---|
...
**문제**: 현재 `hast auto`에서 실패 후 재시도할 때, AI는 같은 프롬프트를 받는다. 이전에 뭘 시도했고 왜 실패했는지 모르기 때문에 같은 실수를 반복할 가능성이 높다.

**해결**: 실패한 attempt의 diff + test output을 저장하고, 다음 attempt 프롬프트에 주입한다.

### 구현 계획

1. `src/hast/core/attempt.py` (~60 lines) — 새 모듈
...
- 기존 CLI (`hast auto`)와 라이브러리 사용 모두에서 효과
...
```python
from hast.core.goals import load_goals, update_goal_status
from hast.core.context import build_context
from hast.core.session import generate_session_log, write_session_log
from hast.core.attempt import save_attempt, load_attempts, clear_attempts
from hast.utils.git import get_head_commit, commit_all, is_dirty, reset_hard, get_changed_files
...
**문제**: goal tree 작성이 hast 사용의 첫 번째 병목. 사람이 작성하든 AI 오케스트레이터가 작성하든, 코드베이스를 이해한 상태에서 적절한 크기로 쪼개야 한다.
...
1. `src/hast/core/pr.py` (~80 lines)
...
- `hast watch` — goals.yaml + sessions/ 폴링, 진행 상황 표시
- rich 기반 라이브 업데이트
